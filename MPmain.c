...
void generateBoard(struct wordStruct sGameStruct[], int nWordCount, int nDimRow, int nDimCol, char board[nDimRow][nDimCol])
{
	int i, j, k;
	int alrExists;
	int randomIndex;
    ...
    for(i = 0; i < nDimRow; i++)
    {
        for(j = 0; j < nDimCol; j++)
        {
            if(nWordCount > 0)
            {
                randomIndex = rand() % nWordCount;
                randomWord = sGameStruct[randomIndex].strWord;
                
                alrExists = 0;
                for(k = 0; k < j && !alrExists; k++)
                {
                    if(board[i][k] == randomWord[0])
                    {
                        alrExists = 1;
                    }
                }

                if(!alrExists) // adds to the board if it does not yet exist
                {
                    board[i][j] = randomWord[0];
                    sGameStruct[randomIndex].strWord[0] = '\0'; // word alr used
                    nWordCount--;
                }
            }
        }
...
void printBoard(struct wordStruct sGameStruct[], int nWordCount, int nDimRow, int nDimCol, char board[nDimRow][nDimCol])
{
    int i, j;
    
    generateBoard(sGameStruct, nWordCount, nDimRow, nDimCol, board);
...
int gameMenu(struct wordStruct sGameStruct[], int nWordCount)
{
	int nDimRow, nDimCol, nDimX;
	int loopCount = 0;
	char fileName[31];
	...
	printf("What dimension will the board be(number of rows)?\n->");
	scanf("%d", &nDimRow);
	printf("\nWhat dimension will the board be(number of columns)?\n->");
	scanf("%d", &nDimCol);
	
	nDimX = nDimRow * nDimCol;
	loopCount++;
	}while(!(nDimRow >= 1 && nDimRow <= 6 && nDimCol >= 1 && nDimCol <= 6 && nDimX <= nWordCount));

	char board[nDimRow][nDimCol];
	if(nWordCount > 0)
		generateBoard(sGameStruct, nWordCount, nDimRow, nDimCol, board);
	
	printBoard(sGameStruct, nWordCount, nDimRow, nDimCol, board);
...
do
{
	...
	{
		case 1:
			gameMenu(sGameStruct, nWordCount);
			break;
		case 2:
			do
			{
