...
{	
	int rV = 1; bool bRepeat; //variable declaration
	do
	{
	bRepeat = false;
	if(!(rV >= 1 && rV <= 9) )
	...
	fflush(stdin); //in case of string or char input
	scanf("%d", &rV);
	} while( !(rV >= 1 && rV <= 10) || bRepeat == true); // while input to guard wrong inputs
...
*/
void boardSize(int *xAxis, int *yAxis, int *nProduct)
{
