...
Ron Fourier B. Alonzo, DLSU ID# 12306002
Cochise John M. King, DLSU ID# 12347574
*********************************************************************************************************/
//	struct for hints
struct hints
{
...
{
	int i = 0, bContinue = 0; 	//variable declaration / temp container declaration
	...
	if(i < 10) 
	{	//do while asks for inputs until nContinue is == 0 or i is equal to 10
		do
		{
					...
					int *nWordCount)
{
	...
	while(strcmp(gStruct[i].strWord, "\0") != 0 && i < 149) //counts how many words are in the struct, was made before the word counting function
		i++;
	do{
...
initGameArr(struct wordStruct sGameStruct[])
{
	...
	for(i=0;i<150;i++)
		strcpy(sGameStruct[i].strWord,"\0"); //sets words to null
	for(i=0;i<150;i++)
						...
						int nWordCount)
{
	...
	do{
		...
		j++; 
	}while(strcmp(sGameStruct[i].sHintPair[j].strRelation,"\0") != 0 && j < 10);
	fprintf(fp, "\n");
	}
	fclose(fp);
			   ...
			   int 	nWordCount)
{
	...
	do //prints the currently selected word until X
	{
		...
		j++;
		}while(strcmp(tempStruct[i].sHintPair[j].strRelation, "\0") != 0 && j < 10);
...
*/
int searchWord(struct wordStruct sGameStruct[] ,
			   int nWordCount)
{
...
{	
	...
	i = searchWord(sGameStruct, nWordCount);
	if(i != -1)
		addTrivia(sGameStruct[i].sHintPair);
	else	printf("\nWord not found\n");
}
				  ...
				  int nWordCount)
{
	...
	for(i = 0; i < nWordCount; i++) //display loop
		printf("%d. %s\n", i + 1, strTemp[i]);
...
/*	modifyWord modifies a word in the array of structs but keeps the struct for the relationship intact
	...
	@param nWordCount - limits for loop used in searchWord
*/
				...
				int nWordCount)
{
	...
	displayAlpha(sGameStruct, nWordCount); //everything below is pretty self explanatory due to the printfs
	...
	if(nTemp != -1)
		...
		{	
			...
			else if(nChoice == 2)
			{
				...
				j++;
				}while(strcmp(sGameStruct[nTemp].sHintPair[j].strRelation, "\0") != 0 && j < 10);
				...
				{
					if(nSelect > j)
							printf("Enter value within range (1 - %d)\n", j);
					nSelect = 0;
				...
				printf("\nEnter hint to replace (ex.Kind of, Part, Height): ");
				fflush(stdin);
				scanf("%[^\n]%*c", sGameStruct[nTemp].sHintPair[nSelect - 1].strRelation); //relation add
		
				printf("\nEnter hint value to replace (ex.Furniture, Leg, Meter): "); 
				fflush(stdin);
				scanf("%[^\n]%*c", sGameStruct[nTemp].sHintPair[nSelect - 1].strRelationValue);//relation value add
...
/*	deleteWord "deletes" a word from the array, in reality it just sets the word and its accompanying relation
	...
	@param nWordCount - for loop limit in searchWord
*/
...
void deleteHint(struct wordStruct sGameStruct[],
				int nWordCount)
{
	int nIndex, i , j = 0, nTemp; //variable initialization
	displayAlpha(sGameStruct, nWordCount);
	...
	j++;
	}while(strcmp(sGameStruct[nIndex].sHintPair[j].strRelation, "\0") != 0 && j < 10);	
	printf("Enter **NUMBER** of hint to DELETE: ");
	scanf("%d", &nTemp); nTemp--;
	
	if(j != 1)
	{
		if(strcmp(sGameStruct[nIndex].sHintPair[nTemp].strRelation, "\0") != 0)
			for(i = nTemp; i < j; i++)
			{	
				if(i != 9)
...
*/
void
printBoard(struct wordStruct tempBoard[10][10],
			int nX,
			int nY, 
			int nLevel)
{
...
*/
void
fillStructBoard(struct wordStruct sGameStruct[], 
				struct wordStruct sTempStruct[10][10], 
				int nX, 
				int nY,
				int nWordCount, 
				int nSize)
{
	...
	time_t randSeed;
	srand( (unsigned) time(&randSeed)); //to randomize trivia
	...
	{	do
		...
		nFindNum = false;
		for(i = 0; i < nSize - 1; i++) // - 1 just so it doesn't get stuck in a loop if it nWordCount == nSize because it'll keep checking itself if it does
...
/*	hintCounter counts hints and returns how many were found, used for the game
	@param hints sHintStruct - struct array to count hints from
	@return amount of hints in sHintStruct
*/
int
hintCounter(struct hints sHintStruct[])
{
	int i = 0;
	while(strcmp(sHintStruct[i].strRelation, "\0") != 0 && i < 10)
		i++;
	return i;
}

/*	wordPick is where the game mostly happens
	@param wordStruct sTempGameStruct - temporary game struct created by fillStructBoard
		 ...
		 int nY)
{
	...
	if(nLevel == nY) //both of these are mutually exclusive
...
*/
void
gamePhase(struct wordStruct sGameStruct[], 
		  int nWordCount)
{
...
//loop for the game
	do
	{
		...
		switch(nMenuVal)
		{
			...
			{	printf("Not enough words to create board!\n\n");}
				break;
			case 2:
				do
				{
				nAdMenVal = adminMenu(nMenuVal);
				switch(nAdMenVal)
				{
					case 1:
						if(nWordCount != 150 )addWord(sGameStruct, &nWordCount); else printf("\nCannot add any more words, memory full!\n");
						break;
					case 2:
						if(nWordCount != 0) addTriviaComp(sGameStruct, nWordCount); else printf("\nImport words first! No words in memory\n");
						break;
					case 3:
						if(nWordCount != 0) modifyWord(sGameStruct, nWordCount); else printf("\nImport words first! No words in memory\n");
						break;
					case 4:
						if(nWordCount != 0) deleteWord(sGameStruct, &nWordCount); else printf("\nImport words first! No words in memory\n");
						break;
					case 5:
						if(nWordCount != 0) deleteHint(sGameStruct, nWordCount); else printf("\nImport words first! No words in memory\n");
						break;
					case 6:
						if(nWordCount != 0) viewWords(sGameStruct, nWordCount); else printf("\nImport words first! No words in memory\n");
						break;
					case 7:
						if(nWordCount != 0) viewHints(sGameStruct, nWordCount);	else printf("\nImport words first! No words in memory\n");
						break;					
					case 8:
						if(nWordCount != 0) exportArray(sGameStruct, nWordCount); else printf("\nImport words first! No words in memory\n");
						break;
					case 9:
				...
				break;
			case 816: matikanetannhauserfunc(); printf("\n");
		}
	}while(nMenuVal != 3);
}